  ...
  try {
    ...
    const [bookings] = await pool.query(
      `SELECT b.id, b.user_id, b.court_id, b.date, b.end_time,
       ...
       AND NOW() > DATE_ADD(CAST(CONCAT(DATE_FORMAT(DATE(b.date), '%Y-%m-%d'), ' ', TIME_FORMAT(b.end_time, '%H:%i:%s')) AS DATETIME), INTERVAL 10 MINUTE)`
    );
  ...
  try {
    ...
    const [bookings] = await pool.query(
      `SELECT b.id, b.user_id, b.total_price, b.date, b.start_time, b.checked_in,
       ...
       AND NOW() > DATE_ADD(
         CAST(CONCAT(DATE_FORMAT(DATE(b.date), '%Y-%m-%d'), ' ', TIME_FORMAT(b.start_time, '%H:%i:%s')) AS DATETIME),
         INTERVAL 45 MINUTE
       )`
    );
